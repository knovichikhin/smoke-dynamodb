...
extension QueryInput {
        ...
        let expressionAttributeValues: [String: AWSDynamoDB.DynamoDbClientTypes.AttributeValue]
        let expressionAttributeNames: [String: String]
        let keyConditionExpression: String
        if let currentSortKeyCondition = sortKeyCondition {
            var withSortConditionAttributeValues: [String: AWSDynamoDB.DynamoDbClientTypes.AttributeValue] = [
                ":pk": AWSDynamoDB.DynamoDbClientTypes.AttributeValue.s(partitionKey)]

            let sortKeyExpression: String
            switch currentSortKeyCondition {
            case .equals(let value):
                withSortConditionAttributeValues[":sortkeyval"] = AWSDynamoDB.DynamoDbClientTypes.AttributeValue.s(value)
            ...
            case .between(let value1, let value2):
                withSortConditionAttributeValues[":sortkeyval1"] = AWSDynamoDB.DynamoDbClientTypes.AttributeValue.s(value1)
                withSortConditionAttributeValues[":sortkeyval2"] = AWSDynamoDB.DynamoDbClientTypes.AttributeValue.s(value2)
            ...
            expressionAttributeNames = ["#pk": AttributesType.partitionKeyAttributeName]
            expressionAttributeValues = [":pk": AWSDynamoDB.DynamoDbClientTypes.AttributeValue.s(partitionKey)]
        }

        let inputExclusiveStartKey: [String: AWSDynamoDB.DynamoDbClientTypes.AttributeValue]?
        if let exclusiveStartKey = exclusiveStartKey?.data(using: .utf8) {
            inputExclusiveStartKey = try JSONDecoder().decode([String: AWSDynamoDB.DynamoDbClientTypes.AttributeValue].self,
